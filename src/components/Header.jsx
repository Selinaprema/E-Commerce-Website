    ...
    <header className="sticky top-0 z-50 transition-all duration-300">
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="w-full md:flex-1 max-w-sm my-4">
            <div className="relative w-full">
              <input
                type="text"
                placeholder="Search..."
                className="w-full px-3 py-2 text-sm border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
              <button
          ...
          <div className="flex items-center justify-end space-x-4 w-full md:auto">
            <button
            ...
            <div className="md:hidden mt-2 bg-white rounded-lg shadow-md p-4 space-y-3 text-amber-950 text-center">
              {navItems.map((item) => (
                <a
                  key={item.id}
                  href={item.link}
                  className="block hover:text-amber-600 font-medium"
                >
                  {item.name}
                </a>
              ))}
